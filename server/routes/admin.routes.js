...
adminRoute.get('/stream',async(req,res)=>{
    try{
        console.log("entered")
        const streams = await fieldsModel.find() || [];
        res.send(streams)
    }
    catch(error){
        res.status(500).send(error)
...
adminRoute.post('/stream',async(req,res)=>{
    try{
        ...
        const doesExist = await fieldsModel.findOne({streamName});
        if(doesExist){
            console.log(doesExist)
            return res.send("this stream already exists");
        }
        const stream = new fieldsModel({streamName});
        await stream.save();
...
adminRoute.put('/stream',async(req,res)=>{
    try{
        ...
        const updatedStream = await fieldsModel.updateOne({_id},{$set:{streamName:streamName}});
...
adminRoute.delete("/stream",async(req,res)=>{
    try{
        ...
        const deletedSteam = await fieldsModel.findByIdAndDelete(streamId)
